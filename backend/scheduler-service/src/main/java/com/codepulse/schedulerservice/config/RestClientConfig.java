 ...
 *
 * ApiServiceClient and WorkerServiceClient each inject this builder and
 * apply their own baseUrl (api-service.base-url / worker-service.base-url)
 * on top of it, so both clients share the same timeout/transport settings
 * while talking to different downstream services.
 */
@Configuration
